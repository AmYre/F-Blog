...
<?php ob_start(); ?>
        ...
        <form action="http://localhost/test/index.php?redir=write_sent" method="post">
        ...
        <?php 
            ...
            while($publication = $show_chapter->fetch())
            {
                ...
                .'<a href="http://localhost/test/index.php?redir=chapter_manager&amp;id='.$publication["id"].'&amp;title='.$publication["title"].'"><button name="read">Gérer ce chapitre</button></a>'.'<br>'.'<br>'.'<br>';
            }
        ?> 
        </div> 
